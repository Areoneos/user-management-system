...
export class FakeBackendInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    function getAccountById() {
      ...
      let account = accounts.find((x: Account) => x.id === accountId);

      if (!account) return error('Account not found');

      // user accounts can get own profile and admin accounts can get all profiles
      const currentAcc = currentAccount();
      if (currentAcc && account.id !== currentAcc.id && !isAuthorized(Role.Admin)) {
        return unauthorized();
      }
    ...
    function newAccountId() {
      return accounts.length ? Math.max(...accounts.map((x: Account) => parseInt(x.id))) + 1 : 1;
    }
